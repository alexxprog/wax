jQuery(function($) {
	...
	function setDims(){
		...
		$(".colct_list .item:not(.opened)").width(Math.ceil(sWdt/100*20));
...
/*
		$(".accs_li").click(function(){
			var ind = $(this).index();
			$(this).removeClass("active");
			var parent = $(this);
			parent.fadeOut("slow");
			$(this).parent().find(".accessory").fadeOut(function() {
				$(this).addClass("hide_item");
				scrollableApi.begin();
			});
			$(this).parent().find(".accs_li:hidden").fadeIn();
			setTimeout(function(){
				parent.nextUntil(".accs_li").each(function(){
					$(this).find(".cl_img").css({"display": "none"});
					$(this).css({"visibility": "visible"}).width(0).removeClass("hide_item").animate({
						width: sWdt/100*33
					}, 500).find(".cl_img").fadeIn("slow", function() {
						console.log(ind+" - "+$(".accessory:first:visible").index());
						scrollableApi.seekTo(ind);
					});
//					console.log($(this));
				});
			}, 500);
